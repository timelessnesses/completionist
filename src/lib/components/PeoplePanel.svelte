...
{#if shareOpen}
	...
	<div class="share-dialog" role="dialog" aria-modal="true" aria-label="Share">
		...
		<div class="link-row">
			<input class="link" readonly value={shareUrl} onclick={(e) => (e.target as HTMLInputElement).select()} />
...
<style>
	...
	.share-head h2 { margin: 0; font-size: 16px; font-weight: 500; color: #1f1f1f; }
	.x {
		display: grid; place-items: center; width: 30px; height: 30px;
		border: 0; border-radius: 50%; background: none; color: #444746; cursor: pointer;
	}
	.x:hover { background: #f0f4f9; }
	.share-desc { font-size: 12.5px; color: #444746; margin: 8px 0 14px; }
	.link-row { display: flex; gap: 8px; }
	.link {
		flex: 1; min-width: 0; font: inherit; font-size: 12.5px; color: #1f1f1f;
		border: 1px solid #c4c7c5; border-radius: 8px; padding: 9px 12px; background: #f8fafd;
	}
	.copy {
		display: inline-flex; align-items: center; gap: 6px;
		border: 0; border-radius: 999px; padding: 0 16px; cursor: pointer;
		background: #0b57d0; color: #fff; font-size: 12.5px; font-weight: 600;
		white-space: nowrap;
	}
