...
class SDATSUImporter extends Importer {
	...
	async login(): Promise<JSESSIONID> {
		...
		const initialJSESSIONID = parseSetCookie(
			(await this.connector.fetch(LOGIN_PAGE)).headers.get('set-cookie') || ''
	...
	async fetchData(): Promise<ActivitiesResponse> {
		...
		return (
			await Promise.all(
				this.getActivityListByHtml(html).map(async (activityId) => {
